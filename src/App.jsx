...
const ACCESS_CODES = ["ASWEEKLY", "ASYEARLY"];
...
function calcStreak(data) {
  const entries = Object.keys(data).filter(k => k.startsWith("day_")).sort().reverse();
  if (!entries.length) return 0;
  let streak = 0;
  let cursor = new Date();
  cursor.setHours(0,0,0,0);
  for (let i = 0; i < entries.length; i++) {
    const d = new Date(entries[i].replace("day_",""));
    d.setHours(0,0,0,0);
...
function markToday(data) {
  const key = "day_" + todayStr();
  if (!data[key]) { const u = {...data, [key]: true}; persist(u); return u; }
  return data;
}
...
export default function AnchoredSteps() {
  ...
  const [unlocked, setUnlocked] = useState(() => load().unlocked || false);
  ...
  const daysComplete = n => DAYS.filter((_,i) => (data["w"+n+"_tr_"+i]||"").trim()).length;
  ...
  const save = () => { persist(data); setFlash(true); setTimeout(() => setFlash(false), 1800); };

  const tryUnlock = () => {
    if (ACCESS_CODES.includes(codeInput.trim().toUpperCase())) {
      const u = {...data, unlocked: true};
      setData(u); persist(u); setUnlocked(true);
    } else {
      setCodeError(true);
  ...
  const checkQuiz = () => {
    ...
    if (result === "pass") {
      const u = {...data, [kk("mem_"+quizVerse.ref)]: true};
      setData(u); persist(u);
    }
  };
  ...
  const exportNotes = () => {
    ...
    ALL_WEEKS.forEach(w => {
      const has = [data["w"+w.week+"_study"],data["w"+w.week+"_rfj"],data["w"+w.week+"_apply"],data["w"+w.week+"_prayer"]].some(v => v && v.trim());
      if (!has) return;
      out += "=".repeat(50) + "\nWEEK " + w.week + ": " + w.title + "\n\n";
      if (data["w"+w.week+"_study"]) out += "STUDY:\n" + data["w"+w.week+"_study"] + "\n\n";
      [0,1,2,3].forEach(i => { if (data["w"+w.week+"_rf"+i]) out += "REFLECTION "+(i+1)+":\n"+data["w"+w.week+"_rf"+i]+"\n\n"; });
      if (data["w"+w.week+"_rfj"]) out += "JOURNAL:\n" + data["w"+w.week+"_rfj"] + "\n\n";
      if (data["w"+w.week+"_apply"]) out += "APPLICATION:\n" + data["w"+w.week+"_apply"] + "\n\n";
      if (data["w"+w.week+"_prayer"]) out += "PRAYER:\n" + data["w"+w.week+"_prayer"] + "\n\n";
    });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(new Blob([out], {type:"text/plain"}));
    a.download = "anchored-steps.txt";
    a.click();
  };
  ...
  if (!unlocked) return (
    <div style={{minHeight:"100vh",background:"linear-gradient(155deg,#0b1825 0%,#132030 55%,#0b1825 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      <div style={{fontSize:36,color:G.gold,marginBottom:16}}>&#9875;</div>
      <div style={{fontFamily:"Cinzel,serif",fontSize:22,fontWeight:600,color:G.cream,marginBottom:4}}>Anchored Steps</div>
      <div style={{fontSize:12,color:G.muted,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:32}}>52 Weeks of Faith in Action</div>
      <div style={{background:"rgba(255,255,255,0.04)",border:"1px solid "+G.goldB,borderRadius:16,padding:"32px",width:"100%",maxWidth:380,textAlign:"center"}}>
        <p style={{fontSize:17,color:G.text,lineHeight:1.7,marginBottom:24,fontStyle:"italic"}}>&#8220;Walk steadily. Stay anchored. Trust God with every step.&#8221;</p>
        <div style={{fontSize:11,color:G.muted,letterSpacing:"0.12em",textTransform:"uppercase",marginBottom:10,fontFamily:"Cinzel,serif"}}>Enter your access code</div>
        <input value={codeInput} onChange={e => { setCodeInput(e.target.value); setCodeError(false); }} onKeyDown={e => e.key === "Enter" && tryUnlock()} placeholder="e.g. ANCHORED2025" style={{width:"100%",background:"rgba(255,255,255,0.06)",border:"1px solid "+(codeError?G.red:G.border),borderRadius:8,color:G.cream,fontSize:16,padding:"12px 14px",outline:"none",fontFamily:"Cinzel,serif",letterSpacing:"0.08em",textAlign:"center",marginBottom:12}} />
        {codeError && <p style={{fontSize:12,color:G.red,marginBottom:10}}>Invalid code &#8212; please check your access email.</p>}
        <button onClick={tryUnlock} style={{width:"100%",background:"linear-gradient(135deg,rgba(180,140,60,0.3),rgba(180,140,60,0.15))",border:"1px solid "+G.goldB,color:G.gold,padding:"12px",borderRadius:8,cursor:"pointer",fontSize:14,fontFamily:"Cinzel,serif",letterSpacing:"0.1em"}}>Unlock Journal</button>
        <div style={{marginTop:20,fontSize:12,color:G.dim,lineHeight:1.6}}>
          Don&#8217;t have a code?{" "}
          <a href="https://buy.stripe.com/9B69ASezO3Xo5uveIb57W00" target="_blank" rel="noreferrer" style={{color:G.gold,textDecoration:"none"}}>Subscribe for $1.50/week &#8594;</a>
        </div>
      </div>
    </div>
  );
      ...
      <header style={{position:"sticky",top:0,zIndex:100,background:"rgba(11,24,37,0.95)",backdropFilter:"blur(20px)",borderBottom:"1px solid "+G.goldB}}>
        <div style={{borderBottom:"1px solid rgba(180,140,60,0.12)",padding:"10px 18px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
            ...
            <span style={{fontFamily:"Cinzel,serif",fontSize:16,fontWeight:600,color:G.cream,letterSpacing:"0.07em"}}>Anchored Steps</span>
            <span style={{width:1,height:13,background:G.goldB,display:"inline-block",margin:"0 2px"}} />
            <span style={{fontSize:10,color:G.muted,letterSpacing:"0.13em",textTransform:"uppercase"}}>52 Weeks of Faith in Action</span>
            {streak > 0 && <span style={{marginLeft:6,background:"rgba(180,140,60,0.15)",border:"1px solid "+G.goldB,borderRadius:12,padding:"2px 10px",fontSize:11,color:G.gold,fontFamily:"Cinzel,serif"}}>&#128293; {streak}</span>}
          </div>
        </div>
                          ...
                          <div style={{flex:1}}>
                            ...
                            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,marginBottom:ae?8:0}}>
                              ...
                              <div style={{display:"flex",gap:6}}>
                                ...
                                <button onClick={() => startQuiz(s)} style={{background:data[kk("mem_"+s.ref)]?"rgba(120,184,120,0.15)":G.purpleF,border:"1px solid "+(data[kk("mem_"+s.ref)]?G.greenB:G.purpleB),color:data[kk("mem_"+s.ref)]?G.green:G.purple,padding:"2px 10px",borderRadius:12,cursor:"pointer",fontSize:11,fontFamily:"Cinzel,serif"}}>
                                  {data[kk("mem_"+s.ref)] ? "✓ Memorized" : "✦ Memorize"}
                                </button>
                              </div>
                            </div>
                            {isOpen && ae && (
                      ...
                      {crossRefs.map((s,i) => (
                        <div key={i} style={{borderLeft:"2px solid "+G.goldB,paddingLeft:14,marginBottom:i<crossRefs.length-1?14:0,paddingBottom:i<crossRefs.length-1?14:0,borderBottom:i<crossRefs.length-1?"1px solid rgba(255,255,255,0.04)":"none"}}>
                          ...
                          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                            ...
                            <button onClick={() => startQuiz(s)} style={{background:data[kk("mem_"+s.ref)]?"rgba(120,184,120,0.1)":G.purpleF,border:"1px solid "+(data[kk("mem_"+s.ref)]?G.greenB:G.purpleB),color:data[kk("mem_"+s.ref)]?G.green:G.purple,padding:"2px 10px",borderRadius:12,cursor:"pointer",fontSize:11,fontFamily:"Cinzel,serif"}}>
                              {data[kk("mem_"+s.ref)] ? "✓ Memorized" : "✦ Memorize"}
                            </button>
                          </div>
                        </div>
